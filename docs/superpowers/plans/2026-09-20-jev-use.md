...
- Produces:
  ...
  - `enumerate_actions(elements: list[Element], inputs: dict[str, str], banned: set[str], *, max_candidates: int = 200) -> list[Action]`
...
200 candidates sits inside demonstrated range — the semantic-find cookbook scores 218 options in one Choice. `max_elements` is 250 so the element cap does not bind before the candidate cap does; at roughly 8 tokens per element line that is about 2,000 tokens of state, or $0.00008 per decision. Cost is not the constraint here, selection quality at large option counts is, and that is still SPEC.md §11.2 — Task 14 measures where quality falls off and these defaults move to whatever it finds.

Truncation past 200 remains order-dependent and remains a known weakness. The escape hatch is the rerank pattern: one Score request over all elements to shortlist, then the selection request over the shortlist. That is a second round trip per step. It stays out of v0.1, but D14 notes it is no longer optional for browser work — a denser page than the one measured will exceed 200.
