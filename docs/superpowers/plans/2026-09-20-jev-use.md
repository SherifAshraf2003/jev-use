...
- Produces:
  ...
  - `parse_tree(root: Any, *, screen_bounds: tuple[int, int] | None = None, max_elements: int = 150, max_label: int = 70) -> list[Element]`
...
- Produces:
  ...
  - `enumerate_actions(elements: list[Element], inputs: dict[str, str], banned: set[str], *, max_candidates: int = 150) -> list[Action]`
  - `CLICKABLE_ROLES: frozenset[str]`, `EDITABLE_ROLES: frozenset[str]`, `FALLBACK_ACTIONS: tuple[Action, ...]`

**Why the cap moved from 80 to 150.** Chrome presented 90 clickable elements across three windows (D10), so SPEC.md's 80 truncated a real screen — and truncation here is silent and arbitrary, since it cuts in tree order, which is not relevance order. An element that never enters the list cannot be selected no matter how obviously correct it is. The semantic-find cookbook scores 218 options in one Choice, so 150 sits inside demonstrated range. This does not close SPEC.md §11.2: the empirical ceiling is still Task 14's measurement, and if it lands below 150 this default comes down with it.

Truncation past 150 remains order-dependent and remains a known weakness. The documented escape hatch, if a screen exceeds it, is the rerank pattern — one Score request over all elements to shortlist, then the selection request over the shortlist. That is a second round trip per step, so it stays out of v0.1 until a measured screen actually needs it.
